  ...
  return (
    <div className="grid place-items-center " >
      <Switch
        checked={enabled}
        onChange={setEnabled}
        className={`${enabled ? 'bg-gray-800' : 'bg-red-700'}
      ...
      >
        ...
        <span
          ...
          className={`${enabled ? 'translate-x-3.5' : 'translate-x-0'}
            pointer-events-none inline-block h-[15px] w-[18px] transform rounded-full bg-white shadow-lg ring-0 transition duration-200 ease-in-out`}
        />
      </Switch>
    </div>
  )
